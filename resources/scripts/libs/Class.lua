        ...
        if type(t[k]) == "function" then
    		...
    		if _supero[k] and type(_supero[k]) == "function" then
				NewClass[k]     = function(that,...)
					...
					ret         = tempfunc(that,...)  --the nil value can not in the middle of unpack() for class
					that._super = s
                    return ret
				end
			else
...
function Class:_instanceof(t,ClassObject)
	local prototype      = getmetatable(t)
	if prototype then
		local pProtoType = prototype.__index
...
function Class:isDerivedBy(cls)
    assert(cls~=nil,"super class can't be nil")
    assert(Class:isClassObject(cls),"super class must be a class")
    return self == cls or self:_instanceof(self,cls)
end
